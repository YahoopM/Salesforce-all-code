...
handleFileChange(event) {
    ...
    if (file && file instanceof Blob && file.type === 'application/pdf') {
        ...
        reader.onloadend = () => {
            ...
            this.showToast('Success', 'PDF file selected and processed successfully!', 'success');
        };
        reader.readAsDataURL(file);  // ✅ This reads the file into base64 format
    } else {
        this.selectedFile = null;
...
handleSubmit() {
    ...
    if (!this.base64Data || !this.fileName) {
        this.showToast('Error', 'Please upload a valid PDF file.', 'error');
        return;
    }
    ...
    .then(recordId => {
        ...
        return uploadSignedPDF({
            selfCertId: this.recordId,
    ...
    .catch(error => {
        ...
        this.showToast('Error', 'Submission failed. Please try again.', 'error');
    });
